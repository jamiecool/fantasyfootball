...
| ESPN sign-in | `espn_login.py` | opens Chrome, you sign in, it writes `espn_credentials.json` (espn_s2 + SWID; gitignored). **Jamie's account joined the league in 2026**, so ESPN refuses the private 2021–2024 seasons to it (`AUTH_LEAGUE_NOT_VISIBLE`) and `leagueHistory` carries no transactions for anyone. Past FAAB needs a leaguemate who was a member then to run this + `fetch_ppp_faab.py`. |
| PPP FAAB | `fetch_ppp_faab.py` | every waiver claim (winner, outbid, failed) and free-agent add per scoring period, from `mTransactions2` → `rawdata/ppp/faab<y>.psv`; the **FAAB tab** groups a week's claims by player and reconstructs each team's remaining $200. Public from 2025; 2021–24 need `espn_credentials.json`. Live feed (`--refresh`). |
